...
public class GraphicsUpdater {
	...
	public static int increaseCompartmentHeigth(ContainerShape containerShape,
		...
		for (Shape shape : containerShape.getChildren()) {
			Iterator<GraphicsAlgorithm> iterator = shape.getGraphicsAlgorithm()
					.getGraphicsAlgorithmChildren().iterator();
	...
	public static void updateContainer(ContainerShape containerShape,
		...
		if (childrenSizeBefore == 2) {
			containerHeightBefore = JPAEditorConstants.COMPARTMENT_MIN_HEIGHT
					+ JPAEditorConstants.COMPARTMENT_BUTTOM_OFFSET;
		}
		containerHeightAfter = containerHeightBefore
				+ (((childrenSizeAfter - childrenSizeBefore) / 2) * JPAEditorConstants.ATTRIBUTES_PLACEMENT_STEP);

		if (!isCollapsed(containerShape))
			containerShape.getGraphicsAlgorithm().setHeight(
					containerHeightAfter);
		else
			containerShape.getGraphicsAlgorithm().setHeight(
					JPAEditorConstants.COMPARTMENT_MIN_HEIGHT);
	}
	...
	public static void updateEntityShape(ContainerShape entityShape) {
		...
		relationShape.getGraphicsAlgorithm().setY(
				getNextCompartmentY(primaryShape));
		basicShape.getGraphicsAlgorithm().setY(
				getNextCompartmentY(relationShape));
	}
	...
	public static void updateEntityHeigth(ContainerShape entityShape) {
		...
		entityShape.getGraphicsAlgorithm().setHeight(
				JPAEditorConstants.ENTITY_MIN_HEIGHT
						+ primaryShape.getGraphicsAlgorithm().getHeight()
						+ relationShape.getGraphicsAlgorithm().getHeight()
	...
	public static int getNextCompartmentY(ContainerShape compartmentShape) {
		...
		return compartmentShape.getGraphicsAlgorithm().getY()
				+ compartmentShape.getGraphicsAlgorithm().getHeight();
	}
	...
	public static ContainerShape getPrimaryShape(ContainerShape entityShape) {
		...
		if (children.size() < PRIMARY_SHAPE_INDEX + 1)
			return null;
		GraphicsAlgorithm alg = children.get(PRIMARY_SHAPE_INDEX)
				.getGraphicsAlgorithm();
		ContainerShape primaryShape = (ContainerShape) Graphiti.getPeService()
				.getActiveContainerPe(alg);
		return primaryShape;
	}

	public static ContainerShape getCompartmentSeparatorShape(
			ContainerShape compartmentShape) {
		GraphicsAlgorithm alg = compartmentShape.getChildren().get(0)
				.getGraphicsAlgorithm();
		ContainerShape primarySeparatorShape = (ContainerShape) Graphiti
				.getPeService().getActiveContainerPe(alg);
	...
	public static ContainerShape getRelationShape(ContainerShape entityShape) {
		...
		if (children.size() < RELATION_SHAPE_CONSTANT + 1)
			return null;
		GraphicsAlgorithm alg = children.get(RELATION_SHAPE_CONSTANT)
				.getGraphicsAlgorithm();
		ContainerShape relationShape = (ContainerShape) Graphiti.getPeService()
				.getActiveContainerPe(alg);
	...
	public static ContainerShape getBasicShape(ContainerShape entityShape) {
		...
		if (children.size() < BASIC_SHAPE_INDEX + 1)
			return null;
		GraphicsAlgorithm alg = children.get(BASIC_SHAPE_INDEX)
				.getGraphicsAlgorithm();
		ContainerShape basicShape = (ContainerShape) Graphiti.getPeService()
				.getActiveContainerPe(alg);
		return basicShape;
	}
	...
	public static boolean isCollapsed(ContainerShape compartmentShape) {
		...
		if (compartmentShape.equals(GraphicsUpdater.getPrimaryShape(container)))
			result = Graphiti.getPeService().getPropertyValue(container,
					JPAEditorConstants.PRIMARY_COLLAPSED);
		else if (compartmentShape.equals(GraphicsUpdater
				.getBasicShape(container)))
			result = Graphiti.getPeService().getPropertyValue(container,
					JPAEditorConstants.BASIC_COLLAPSED);
		else if (compartmentShape.equals(GraphicsUpdater
				.getRelationShape(container)))
			result = Graphiti.getPeService().getPropertyValue(container,
	 ...
	 */
	public static boolean setCollapsed(ContainerShape compartmentShape,
			boolean collapsed) {
		ContainerShape container = compartmentShape.getContainer();
		if (compartmentShape.equals(GraphicsUpdater.getPrimaryShape(container))) {
	...
	public static void updateHeader(ContainerShape entityShape,
	...
	private static Text getHeaderText(ContainerShape entityShape) {
		...
		while (shIt.hasNext()) {
			...
			String shapeType = Graphiti.getPeService().getPropertyValue(
					headerShape, JPAEditorConstants.PROP_SHAPE_TYPE);
	...
	/**
	 * Rename the relevant attribute's pictogram element and relink this pictogram element to the renamed attrbiute.
	 * @param fp - the jpa diagram editor feature provider
	 * @param oldAttribute - old persistent attribute
	 * @param renamedAttribute - the newly renamed persistent attribute
	 * @param newAttributeName -the new name of the persistent attribute
	 */
	public static void renameAttributePictogramElement(IJPAEditorFeatureProvider fp, PersistentAttribute oldAttribute, PersistentAttribute renamedAttribute, final String newAttributeName) {
		PictogramElement pe = fp.getPictogramElementForBusinessObject(oldAttribute);
		...
		ted.getCommandStack().execute(new RecordingCommand(ted) {
			@Override
			protected void doExecute() {
				((Text) alg).setValue(newAttributeName);
			}
		});
		
		linkNewBoToPict(oldAttribute, pe, fp, renamedAttribute);
	}
}
